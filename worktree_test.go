...
func (s *WorktreeSuite) TestPullNonFastForward(c *C) {
	...
	_, err = w.Pull(&PullOptions{})
	c.Assert(err, Equals, ErrMergeCommitNeeded)
}
...
func (s *RepositorySuite) TestPullAdd(c *C) {
	...
	_, err = w.Pull(&PullOptions{RemoteName: "origin"})
	c.Assert(err, IsNil)
...
func (s *WorktreeSuite) TestAddUntracked(c *C) {
	...
	e, err := idx.Entry("foo")
	c.Assert(err, IsNil)
	c.Assert(e[0].Mode, Equals, filemode.Executable)
...
func (s *WorktreeSuite) TestAddModified(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(e[0].Mode, Equals, filemode.Regular)
